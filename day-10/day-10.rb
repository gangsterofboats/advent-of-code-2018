...
def part_one(input)
  ...
  1.upto(time_to_take) do |j|
    lights_map = Array.new(200) {Array.new(300, '.')} if j == (time_to_take - 1)
    lights.each do |light|
      light[:position][0] += light[:velocity][0]
      light[:position][1] += light[:velocity][1]
      x_index = light[:position][0]
      y_index = light[:position][1]
      lights_map[y_index][x_index] = '#' if j == (time_to_take - 1)
    end
    if j == (time_to_take - 1)
      parr = []
